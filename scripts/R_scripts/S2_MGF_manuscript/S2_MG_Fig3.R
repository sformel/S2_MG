...
#Last updated 31 Aug 2020 by Steve Formel
...
p.list.lm <- lapply(p.list[c(1:2)], lm, formula = value^2 ~ log(Total.relevant.PAHs))
p.list.lm[3] <- lapply(p.list[c(3)], lm, formula = value ~ log(Total.relevant.PAHs))

p.list.trans <- c("squared", "squared", "none")
...
#corr.list[c(1:2)] <- lapply(p.list[c(1:2)], function(x){
#  cor.test(x$value^2, log(x$Total.relevant.PAHs), method = 'spearman', exact = FALSE)
#  })

#corr.list[3] <- lapply(p.list[3], function(x){
 # cor.test(x$value, log(x$Total.relevant.PAHs), method = 'spearman', exact = FALSE)
#})
...
#PC1-----
#Simpson's doesn't need to be transformed
p.list.lm <- lapply(p.list[c(1:2)], lm, formula = value^2 ~ PC1)
p.list.lm[3] <- lapply(p.list[c(3)], lm, formula = value ~ PC1)
...
# 
# corr.list[3] <- lapply(p.list[3], function(x){
#   cor.test(x$value, x$PC1, method = 'spearman', exact = FALSE)
# })
# 
# 
# corr.list <- lapply(corr.list, function(x){
